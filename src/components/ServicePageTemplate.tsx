...
export type ForWhoItem = { title: string; body: string };
export type StepItem = { num: string; title: string; body: string; cta: string; img: string };
export type FaqItem = { q: string; a: string };

type Props = {
  badge: string;
  heroTitle: string[];       // each string = one line
  heroImg: string;
  forWho: ForWhoItem[];
  steps: StepItem[];
  faqs?: FaqItem[];
...
const defaultFaqs: FaqItem[] = [
  ...
  { q: "What results can I realistically expect?", a: "Most clients report measurable changes within 4–6 weeks — feedback from managers, more presence in meetings, increased confidence in high-stakes situations." },
];
...
const ServicePageTemplate = ({ badge, heroTitle, heroImg, forWho, steps, faqs }: Props) => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative w-full overflow-hidden h-[88vh] md:h-[92vh] lg:h-[96vh]">
        ...
        <img src={heroImg} alt={heroTitle.join(" ")} className="absolute inset-0 z-0 w-full h-full object-cover grayscale contrast-110" style={{ objectPosition: "center 30%" }} />
        ...
        <div className="relative z-20 flex flex-col items-center justify-center h-full text-center px-6">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7 }} className="max-w-5xl">
            <p className="text-primary-foreground/85 text-xs tracking-[0.25em] uppercase font-sans mb-4">{badge}</p>
            <h1 className="mx-auto max-w-5xl text-4xl md:text-8xl lg:text-[120px] uppercase font-display text-primary-foreground mb-8 tracking-tight leading-none">
              {heroTitle.map((line, i) => (
                <span key={i}>{line}{i < heroTitle.length - 1 ? <br /> : <span className="text-accent">.</span>}</span>
              ))}
            </h1>
      ...
      <section className="bg-background px-6 md:px-16 lg:px-28 py-24 md:py-32 relative overflow-hidden">
        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 8, repeat: Infinity }} className="absolute rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, #f3e200 0%, transparent 65%)", width: 400, height: 400, top: -150, right: -150 }} />
        <div className="relative z-10">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-14">
            ...
            <h2 className="font-display text-3xl md:text-5xl text-foreground leading-tight">This program was built for you if…</h2>
          </motion.div>
          <div className="grid sm:grid-cols-2 gap-px bg-border">
            {forWho.map((item, i) => (
              <motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="group bg-background p-8 md:p-10 hover:bg-accent/5 transition-colors duration-400 cursor-default">
      ...
      <section style={{ background: "#0d0d0d" }}>
        {steps.map((step) => (
          <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="group grid md:grid-cols-2 border-t cursor-default transition-all duration-500" style={{ borderColor: "rgba(255,255,255,0.07)" }}
          ...
          >
            <div className="flex flex-col justify-center px-8 md:px-16 lg:px-20 py-16">
              ...
              <a href="/contact" className="inline-flex items-center gap-2 text-xs tracking-[0.2em] uppercase font-sans text-white/40 border-b border-white/20 pb-1 w-fit group-hover:text-black group-hover:border-black/40 transition-all duration-300">
                {step.cta} <ArrowUpRight size={12} />
              </a>
            </div>
            <div className="overflow-hidden flex items-end justify-center" style={{ background: "#0d0d0d" }}>
              <img src={step.img} alt={step.title} className="w-full h-[360px] md:h-[520px] object-cover object-top transition-all duration-700" style={{ mixBlendMode: "screen", filter: "grayscale(1) contrast(1.1)" }} />
            </div>
          </motion.div>
        ))}
      </section>
